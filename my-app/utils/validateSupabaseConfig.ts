...
export const validateSupabaseConfig = (): {
  ...
  if (!supabaseUrl) {
    errors.push(
      'EXPO_PUBLIC_SUPABASE_URL is missing from environment variables',
    );
  } else {
    // Check URL format
    if (supabaseUrl.includes('...')) {
      errors.push(
        'Supabase URL appears to be truncated (contains "..."). Please provide the complete URL.',
      );
    }
  ...
  if (!supabaseAnonKey) {
    errors.push(
      'EXPO_PUBLIC_SUPABASE_ANON_KEY is missing from environment variables',
    );
  } else {
    ...
    if (supabaseAnonKey.includes('...')) {
      errors.push(
        'Supabase anon key appears to be truncated (contains "..."). Please provide the complete key.',
      );
    }
    ...
    if (supabaseAnonKey.length < 100) {
      errors.push(
        'Supabase anon key appears to be too short. Please verify the complete key.',
      );
    }
  }
